  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 glass-nav py-2 px-4 border-t border-[#232841] backdrop-blur-lg shadow-2xl">
      <div className="max-w-md mx-auto flex justify-between">
        ...
        {/* Notification menu */}
        <Link href="/notification" className="flex flex-col items-center group">
          <div className={`w-10 h-10 rounded-full flex items-center justify-center mb-1 transition-all duration-200 ${
            isActive('/notification')
              ? "bg-gradient-to-br from-[#FFC452] to-[#5D4FFF] shadow-[0_0_12px_#FFC45299]"
              : "bg-[#232841]"
          }`}>
            <svg xmlns="http://www.w3.org/2000/svg" className={`h-5 w-5 ${isActive('/notification') ? 'text-[#FFC452]' : 'text-gray-400'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <span className={`text-xs font-medium ${isActive('/notification') ? 'text-[#FFC452]' : 'text-gray-400'}`}>
            Notification
          </span>
        </Link>
      </div>
